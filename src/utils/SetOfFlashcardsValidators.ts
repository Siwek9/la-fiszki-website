...
export const ReadyToBeExported = st.record({
    name: st.string({minLength: 1, maxLength: 50, trim: true}),
    ...
    flashcards: st.array(
        st.record({
            front: st.array(st.string({minLength: 1, maxLength: 500, trim: true}), {minLength: 1}),
            back: st.array(st.string({minLength: 1, maxLength: 500, trim: true}), {minLength: 1}),
        }),
        {minLength: 1}
    ),
});
